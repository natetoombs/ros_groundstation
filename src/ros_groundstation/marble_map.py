...
class MarbleMap(QWidget):
    def __init__(self, gps_dict, blankname, parent=None):
        ...
        self.extension_painters = []

    def add_extension_painter(self, painter):
        self.extension_painters.append(painter)
    ...
    def show_context_menu(self, point):
        ...
        clickX = point.x()
        clickY = point.y()
        lon = GoogleMapPlotter.pix_to_rel_lon(
            self.GMP.west, clickX, self.GMP.zoom)
        lat = GoogleMapPlotter.pix_to_rel_lat(
            self.GMP.north, clickY, self.GMP.zoom)
    ...
    def replace_waypoints(self, n, e, d):
        PPPub.publishWaypointShort(n, e, set_current=True)
    ...
    def change_home(self, map_name):
        ...
        # FOR INTERFACING WITH SUBSCRIBERS
        self.latlonalt = [self.latlon[0], self.latlon[1], 0.0]
        if not InitSub.with_init:
            InitSub.updateInitLatLonAlt(self.latlonalt)
        self.zoom = self._gps_dict[self._home_map][1]
        self.GB = Geobase(self.latlon[0], self.latlon[1])
        if self.GMP is None:
            self.GMP = GoogleMapPlotter(
                self._gps_dict, self.w_width, self.w_height, self._home_map, self.blankname)
        else:
        ...
        if not self._mouse_attentive:  # we won't do anything with movement in point-and-click mode
            if QMouseEvent.buttons():  # in act of dragging
                ...
                if self.mouse_event_counter > self.counter_limit:
                    ...
                    lon_incremented = GoogleMapPlotter.pix_to_rel_lon(
                        self.GMP.center.lon, delta_x, self.GMP.zoom)
                    delta_y = self.lat_multiplier * qpoint_delta.y()
                    lat_incremented = GoogleMapPlotter.pix_to_rel_lat(
                        self.GMP.center.lat, delta_y, self.GMP.zoom)
    ...
    def paintEvent(self, QPaintEvent):
        ...
        painter.drawLine(self.GMP.width / 2, self.GMP.height /
                         2 - 8, self.GMP.width / 2, self.GMP.height / 2 + 8)
        painter.drawLine(self.GMP.width / 2 - 8, self.GMP.height / 2,
                         self.GMP.width / 2 + 8, self.GMP.height / 2)
        ...
        for extension_painter in self.extension_painters:
            extension_painter(painter, self)
        painter.end()

    # draws gridlines at 10-meter increments
    def draw_grid(self, painter):
        painter.fillRect(QRect(0, self.GMP.height - 20,
                               self.GMP.width, self.GMP.height), Qt.white)
        painter.fillRect(QRect(0, 0, 50, self.GMP.height - 20), Qt.white)
        painter.setPen(QPen(QBrush(Qt.black), 1.5, Qt.DotLine, Qt.RoundCap))
        pixels_per_dist = self.grid_dist * 2 ** self.GMP.zoom / \
            (156543.03392 * cos(radians(InitSub.init_latlonalt[0])))

        x_min, x_offset = divmod(
            GoogleMapPlotter.rel_lon_to_rel_pix(
                InitSub.init_latlonalt[1], self.GMP.west, self.GMP.zoom),
            pixels_per_dist)
        ...
        y_min, y_offset = divmod(
            GoogleMapPlotter.rel_lat_to_rel_pix(
                self.GMP.south, InitSub.init_latlonalt[0], self.GMP.zoom),
            pixels_per_dist)
        ...
        for i, x_line in enumerate(range(x_min, x_max + 1)):
            ...
            if self.GMP.zoom > 17:
                ...
                painter.drawText(text_point, QString('%d m' %
                                                     (x_line * self.grid_dist)))
        ...
        for i, y_line in enumerate(range(y_min, y_max + 1)):
            ...
            if self.GMP.zoom > 17:
                ...
                painter.drawText(text_point, QString('%d m' %
                                                     (y_line * self.grid_dist)))

    def draw_waypoints(self, painter):
        painter.setPen(QPen(QBrush(Qt.darkRed), 2.5,
        ...
        for waypoint in WaypointSub.waypoints:
            ...
            if x >= 0 and x <= self.GMP.width and y >= 0 and y <= self.GMP.height:
                ...
                if waypoint.chi_valid:
                    painter.drawLine(
                        x, y, x + 2 * rad * sin(waypoint.chi_d), y - 2 * rad * cos(waypoint.chi_d))
    ...
    def draw_path(self, painter):
        ...
        if PPSub.search_approved:
            painter.setPen(QPen(QBrush(Qt.magenta), 2.0,
                                Qt.SolidLine, Qt.RoundCap))
        else:
            painter.setPen(QPen(QBrush(Qt.gray), 2.0,
                                Qt.SolidLine, Qt.RoundCap))
        ...
        if PPSub.landing_approved:
            painter.setPen(QPen(QBrush(Qt.cyan), 2.0,
                                Qt.SolidLine, Qt.RoundCap))
        else:
            painter.setPen(QPen(QBrush(Qt.gray), 2.0,
                                Qt.SolidLine, Qt.RoundCap))
    ...
    def draw_currentpath(self, painter):
        ...
        if PathSub.path_type == 1:  # line path
            ...
            if pt_c[0] >= 0 and pt_c[0] <= self.GMP.width and pt_c[1] >= 0 and pt_c[1] <= self.GMP.height:
                R_pix = R * 2 ** self.GMP.zoom / \
                    (156543.03392 * cos(radians(c[0])))
                painter.drawEllipse(
                    pt_c[0] - R_pix, pt_c[1] - R_pix, 2 * R_pix, 2 * R_pix)
    ...
    def draw_debug_text(self, painter, text, width=200):
        ...
        rect = fm.boundingRect(
            QRect(0, 0, width, 100000), Qt.TextWordWrap, text)
    ...
    def draw_single_extended_path(self, painter, path):
        ...
        if path.is_line:  # line path
            ...
            if 0 <= pt_c[0] <= self.GMP.width and 0 <= pt_c[1] <= self.GMP.height:
                R_pix = R * 2 ** self.GMP.zoom / \
                    (156543.03392 * cos(radians(c[0])))
                painter.drawArc(pt_c[0] - R_pix, pt_c[1] - R_pix,
                                2 * R_pix, 2 * R_pix, orbit_start_qt, orbit_span_qt)
    ...
    def draw_plane(self, painter):
        ...
        if x >= 0 and x <= self.GMP.width and y >= 0 and y <= self.GMP.height:
            ...
            pt_3_x = x
            pt_3_y = y
            pt_4_x = x + self.rotate_x(-self.plane_w /
                                       2, -self.plane_h / 4, chi)
            pt_4_y = y - self.rotate_y(-self.plane_w /
                                       2, -self.plane_h / 4, chi)
            pt_5_x = x + self.rotate_x(self.plane_w /
                                       2, -self.plane_h / 4, chi)
            pt_5_y = y - self.rotate_y(self.plane_w /
                                       2, -self.plane_h / 4, chi)
            pt_6_x = x + self.rotate_x(-self.plane_w /
                                       4, -2 * self.plane_h / 5, chi)
            pt_6_y = y - self.rotate_y(-self.plane_w /
                                       4, -2 * self.plane_h / 5, chi)
            pt_7_x = x + self.rotate_x(self.plane_w /
                                       4, -2 * self.plane_h / 5, chi)
            pt_7_y = y - self.rotate_y(self.plane_w /
                                       4, -2 * self.plane_h / 5, chi)
            ...
            painter.setPen(QPen(QBrush(Qt.black), 2.5,
                                Qt.SolidLine, Qt.RoundCap))
            painter.drawLine(pt_1_x, pt_1_y, pt_2_x, pt_2_y)
            painter.drawLine(pt_3_x, pt_3_y, pt_4_x, pt_4_y)
            painter.drawLine(pt_3_x, pt_3_y, pt_5_x, pt_5_y)
