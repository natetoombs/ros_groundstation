...
class InitSub():
    ...
    def state_callback(state):
        lla = ecef2lla(state.position) # Only works in ECEF
        InitSub.init_latlonalt[0] = lla[0]
        InitSub.init_latlonalt[1] = lla[1]
        InitSub.init_latlonalt[2] = lla[2]
        print(InitSub.init_latlonalt)
        InitSub.GB = Geobase(InitSub.init_latlonalt[0], InitSub.init_latlonalt[1])
    ...
    def updateGPSInitTopic(new_topic):
        ...
        InitSub.gi_sub = rospy.Subscriber(InitSub.gps_init_topic, GNSS, InitSub.state_callback)
...
class StateSub(): # TODO: Use Odometry Message
    state_sub = None
    state_topic = None
    lat = 0.0
    lon = 0.0
    ...
    def state_callback(state):
        if InitSub.enabled:
            n = state.pose.pose.position.x #position[0]
            e = state.pose.pose.position.y #position[1]
            d = state.pose.pose.position.z #position[2]
            StateSub.lat, StateSub.lon, StateSub.alt = InitSub.GB.ned_to_gps(n, e, d)
            # StateSub.alt -= InitSub.init_latlonalt[2]
            # StateSub.chi = fmod(state.chi, 2 * pi) # FIXME
            # StateSub.Va = state.Va # FIXME
...
class GPSDataSub():
    ...
    def callback_GPS(gps_data):
        GPSDataSub.numSat = 1 #gps_data.num_sat
...
class ConInSub(): # FIXME: No controller inners
    con_in_sub = None
...
class ConComSub(): # FIXME: No airspeed command
    con_com_sub = None
